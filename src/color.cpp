#include "main.h"

const color_t COLOR_WHITE = { 255, 255, 255 };
const color_t COLOR_RED = { 236, 100, 75 };
const color_t COLOR_GREEN = { 135, 211, 124 };
const color_t COLOR_BLACK = { 0, 0, 0 };
const color_t COLOR_BACKGROUND = { 65, 105, 225 };
const color_t COLOR_GOLD = { 255, 215, 0 };
const color_t COLOR_GREY = {  128, 128, 128 };
const color_t COLOR_ORANGE = { 244, 114, 9 };
const color_t COLOR_BLUE = { 0, 191, 255 };
const color_t COLOR_PURPLE = { 	72,61,139};
const color_t COLOR_PINK = { 	255,20,147};
const color_t COLOR_SPRING = { 	0, 250, 154 };
const color_t COLOR_DARKBLUE = { 0, 0, 139 };
const color_t COLOR_SKYBLUE = { 0, 191, 255 };
const color_t COLOR_BURLYWOOD = { 222, 184, 135 }; //Sand Brown
const color_t COLOR_SEABLUE = { 0, 119, 190 };
const color_t COLOR_ORANGERED = { 255, 69, 0 };
const color_t COLOR_LIMEGREEN = { 50, 205, 50 };
const color_t COLOR_YELLOW = { 255, 255, 0 };
const color_t COLOR_FORESTGREEN = { 34, 139, 34};
const color_t COLOR_BROWN = { 102, 51, 0 };
